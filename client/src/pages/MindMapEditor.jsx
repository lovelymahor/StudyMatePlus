...
function StudyNode({ id, data }) {
  const { label, subject, color, onToggleCollapse, collapsed, matchesQuery } =
    data;
  const bg = color || SUBJECTS[subject] || SUBJECTS.Default;

  return (
    <div
      className={`node-inner ${matchesQuery ? "highlight" : ""}`}
      style={{ background: bg }}
    >
      <div>{label}</div>
...
export default function MindMapEditor() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        // FIX: Added comment to fix 'no-empty' rule
        // Errors while parsing are ignored.
      }
    }
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      "smp_map_v2",
      JSON.stringify({ nodes, edges, collapsed: Array.from(collapsed) })
    );
    if (toast) {
  ...
  const hiddenTargets = useMemo(
    () => computeHiddenTargets(collapsed, edges),
    [collapsed, edges]
  );
  ...
  const visibleEdges = useMemo(() => {
    return edges.filter(
      (e) =>
        !hiddenTargets.has(e.source) &&
        !hiddenTargets.has(e.target) &&
        !collapsed.has(e.source)
    );
  ...
  const addChild = useCallback(
    (subject = "Default") => {
      ...
      const pos = {
        ...
        y: base.position.y + (Math.random() * 160 - 80),
      };
      ...
      const newNode = {
        ...
        type: "study",
        position: pos,
        data: { label: `${subject} Topic`, subject, color, collapsed: false },
      };
  ...
  const applyTemplate = (type) => {
    if (type === "exam") {
      ...
      const n2 = {
        ...
        type: "study",
        position: { x: 60, y: 260 },
        data: { label: "Math", subject: "Math", color: SUBJECTS.Math },
      };
      const n3 = {
        id: "3",
        type: "study",
        position: { x: 250, y: 260 },
        data: { label: "Science", subject: "Science", color: SUBJECTS.Science },
      };
      const n4 = {
        id: "4",
        type: "study",
        position: { x: 440, y: 260 },
        data: { label: "History", subject: "History", color: SUBJECTS.History },
      };
    ...
    if (type === "planner") {
      const root = {
        ...
        type: "study",
        position: { x: 250, y: 90 },
        data: { label: "Today", subject: "Default", color: SUBJECTS.Default },
      };
      const morn = {
        id: "m",
        type: "study",
        position: { x: 80, y: 250 },
        data: { label: "Morning", subject: "Default", color: "#6c8cff" },
      };
      const noon = {
        id: "n",
        type: "study",
        position: { x: 250, y: 250 },
        data: { label: "Afternoon", subject: "Default", color: "#64d2b4" },
      };
      const eve = {
        id: "e",
        type: "study",
        position: { x: 420, y: 250 },
        data: { label: "Evening", subject: "Default", color: "#ff9e6e" },
      };
  ...
  const exportJSON = () => {
    const blob = new Blob(
      [
        JSON.stringify(
          { nodes, edges, collapsed: Array.from(collapsed) },
          null,
          2
        ),
      ],
  ...
  return (
    <div className={`mindmap ${dark ? "dark" : ""}`}>
      ...
      >
        <div className="group">
          <button onClick={() => applyTemplate("exam")}>
            <Layers size={16} /> Exam Prep
          </button>
          <button onClick={() => applyTemplate("research")}>
            <Layers size={16} /> Research
          </button>
          <button onClick={() => applyTemplate("planner")}>
            <Layers size={16} /> Planner
          </button>
        </div>
        <div className="group">
      ...
      <div className="canvas" ref={wrapperRef}>
        ...
        {/* Autosave toast */}
        <div id="save-toast" className="toast">
          💾 Saved
        </div>
      </div>
          ...
          <>
              ...
              <div className="field" style={{ flex: 1 }}>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    updateSelected({
                      subject: subj,
                      color: SUBJECTS[subj] || SUBJECTS.Default,
                    });
                  }}
                >
            ...
            <div className="row">
              <button
                ...
                onClick={() => addChild(selectedNode.data.subject || "Default")}
              >
